...
import { fetchClientInfo, login, loginByCode, register, SsoApiError } from './sso-api'
...
describe('loginByCode', () => {
  it('posts camelCase JSON with a single account + code field (not email/phone split)', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      Response.json({ redirectUrl: 'http://demo.localhost:3000/callback?code=abc&state=abc' }),
    )
    vi.stubGlobal('fetch', fetchMock)

    const result = await loginByCode({ ...authorizeParams, account: 'demo@aieducenter.com', code: '246810' })
  ...
  it('passes the account through verbatim for a phone (no client-side splitting)', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(Response.json({ redirectUrl: 'http://demo.localhost:3000/callback?code=abc' }))
    vi.stubGlobal('fetch', fetchMock)

    await loginByCode({ ...authorizeParams, account: '13800138000', code: '246810' })
  ...
  it('routes a wrong code (400 CODE_INVALID) to the code field with the backend message', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
    ...
    const error = await loginByCode({ ...authorizeParams, account: 'a@b.c', code: '000000' }).catch(
      (e: unknown) => e,
    )
  ...
  /**
   * 防枚举 AC 核心（后端已保证，前端文案对齐）：「账号不存在」与「验证码错误」后端返同一
   * 400 CODE_INVALID（status/message 完全一致）→ 前端必须产出同一错误（同字段、同文案），不得区分。
   * 这里喂给前端两份相同的后端响应，断言前端不会在两场景间分叉。
   */
  it('treats unknown-account and wrong-code identically (anti-enumeration: same message + same field)', async () => {
    const wrongCodeResponse = Response.json(
      { code: 400, message: '验证码错误', data: null },
      { status: 400 },
    )
    const unknownAccountResponse = Response.json(
      { code: 400, message: '验证码错误', data: null },
      { status: 400 },
    )
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValueOnce(wrongCodeResponse).mockResolvedValueOnce(unknownAccountResponse),
    )

    const wrongCode = await loginByCode({ ...authorizeParams, account: 'real@aieducenter.com', code: '000000' }).catch(
      (e: unknown) => e,
    )
    const unknownAccount = await loginByCode({ ...authorizeParams, account: 'ghost@aieducenter.com', code: '246810' }).catch(
      (e: unknown) => e,
    )

    expect((wrongCode as SsoApiError).message).toBe((unknownAccount as SsoApiError).message)
    expect((wrongCode as SsoApiError).field).toBe('code')
    expect((unknownAccount as SsoApiError).field).toBe('code')
  })

  it('maps a disabled account (401) to the banner with the backend message (no field)', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        Response.json({ code: 401, message: '账号已停用', data: null }, { status: 401 }),
    ...
    const error = await loginByCode({ ...authorizeParams, account: 'a@b.c', code: '246810' }).catch(
      (e: unknown) => e,
    )
  ...
  it('maps a locked account (401) to the banner with the backend message (no field)', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        Response.json({ code: 401, message: '账号已锁定', data: null }, { status: 401 }),
  ...
  it('falls back to a generic copy on unexpected statuses (e.g. 500, no field)', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(Response.json({ code: 500, message: 'Internal Server Error' }, { status: 500 })),
    )
